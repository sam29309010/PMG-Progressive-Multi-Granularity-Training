...
def test(net, criterion, batch_size):
    ...
    testset = torchvision.datasets.ImageFolder(root='../CUB200/testing_images',
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        if batch_idx % 50 == 0:
            print(
                'Step: %d | Loss: %.3f | Acc: %.3f%% (%d/%d) |Combined Acc: %.3f%% (%d/%d)' %
                (batch_idx,
                 test_loss /
                 (
                     batch_idx +
